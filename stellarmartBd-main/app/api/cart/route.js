...
async function ensureDbConnected(operation) {
  try {
    ...
    console.warn(`Cart ${operation} fallback: database unavailable`, error?.message || error);
    return false;
  }
}
...
export async function PUT(request) {
  try {
    const dbReady = await ensureDbConnected('PUT');
    if (!dbReady) {
      return NextResponse.json(
        { error: 'Cart service temporarily unavailable' },
        { status: 503 }
...
export async function DELETE(request) {
  try {
    const dbReady = await ensureDbConnected('DELETE');
    if (!dbReady) {
      return NextResponse.json(
        { error: 'Cart service temporarily unavailable' },
        { status: 503 }
