        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-9 space-y-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { label: 'Edit Profile', desc: 'Update your personal info', icon: FaEdit, href: '/dashboard/profile', color: 'bg-blue-600' },
                { label: 'Shipping Info', desc: 'Manage delivery addresses', icon: FaMapMarkerAlt, href: '/dashboard/addresses', color: 'bg-green-600' },
                { label: 'Security', desc: 'Change password & privacy', icon: FaCog, href: '/dashboard/settings', color: 'bg-purple-600' },
              ].map((box, i) => (
                <Link key={i} href={box.href} className="bg-white p-6 rounded-3xl border border-gray-100 hover:border-blue-200 transition-all group shadow-sm">
                  <div className={`w-12 h-12 ${box.color} text-white rounded-2xl flex items-center justify-center text-xl mb-4 shadow-lg shadow-gray-200 group-hover:scale-110 transition-transform`}>
                    <box.icon />
                  </div>
                  <h3 className="font-black text-gray-900 mb-1">{box.label}</h3>
